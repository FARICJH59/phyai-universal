...
class ReceiptChainEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if self.chain_digest != self.compute_digest(
    ...
    def compute_digest(
        ...
        canonical = "|".join(
            (
                ...
                str(sequence),
                result_digest,
                previous_receipt_digest or "GENESIS",
            )
        )
...
class ReceiptChainVerifier:
    ...
    def verify(self, entries: tuple[ReceiptChainEntry, ...]) -> ReceiptChainVerification:
        ...
        return ReceiptChainVerification(
            True,
            len(entries),
            previous.chain_digest if previous else None,
            "valid",
        )


class ReceiptChain:
    """Builds append-only receipt links from already-verified execution evidence."""
